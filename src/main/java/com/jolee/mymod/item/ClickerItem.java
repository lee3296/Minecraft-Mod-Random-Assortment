...
public class ClickerItem extends Item {
    ...
    public boolean isFoil(ItemStack stack) {
    	//makes item have glowing enchanted effect
        final CompoundTag nbt = stack.getOrCreateTag();
        return nbt.contains(MyMod.MOD_ID, Tag.TAG_COMPOUND)
                && nbt.getCompound(MyMod.MOD_ID).contains("ContainedBlock", Tag.TAG_COMPOUND);
    }

    @Override
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        //same as useOn method without the placement implementation
        ...
        if (!stack.getOrCreateTag().contains(MyMod.MOD_ID, Tag.TAG_COMPOUND)) {
            stack.getOrCreateTag().put(MyMod.MOD_ID, new CompoundTag());
            return InteractionResultHolder.fail(stack);
        }

        final CompoundTag nbt = stack.getOrCreateTag().getCompound(MyMod.MOD_ID);
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (!canInteract(player, pos))
        	//if player can't interact with item, return a fail
            return InteractionResult.FAIL;

        if (!stack.getOrCreateTag().contains(MyMod.MOD_ID, Tag.TAG_COMPOUND)) {
        	//check if the tag contains a compound called MOD_ID
        	//if not create the compound tag
            stack.getOrCreateTag().put(MyMod.MOD_ID, new CompoundTag());
        }
        ...
        if (!nbt.contains("ContainedBlock", Tag.TAG_COMPOUND)) {
            if (!state.isAir()) {
            	//set block to air if the block is not air to remove it
        	...
        	//check if state can be replaced or not
        	//set block at the position of the nbt we read
